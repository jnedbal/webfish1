...
if firsthtml == 1
    unix_cmd = ['ssh -o StrictHostKeyChecking=no -p ' webserver.port ...
    	' ' webserver.name '@' webserver.address ' ''rm -f ' ...
        webserver.directory 'uploads/' tasks{1} '.html'''];
    [st, w] = unix(unix_cmd);
    error_msg(process, st, {'Failed sending html file to the server', w});
    firsthtml = 0;
end
if sendhtml == true && ~isequal(webserver.html, '.')
    unix_cmd = ['scp -o StrictHostKeyChecking=no -P ' webserver.port ...
    	' ' fname ' ' webserver.name '@' webserver.address ':' ...
        webserver.directory 'uploads'];
    [st, w] = unix(unix_cmd);
    error_msg(process, st, {'Failed sending html file to the server', w});
end